...
- 研究到执行一体化工作台现在已完成第一步：
  ...
  - 现在还能直接配置研究标的、研究周期和样本长度，保存后下一轮训练、推理和回测会按这里的范围运行
- 研究到执行一体化工作台现在已完成第二步：
  - 新增特征工作台 `/features`
  - 新增特征工作台接口 `GET /api/v1/features/workspace`
  - 页面会直接显示因子分组、主判断因子、辅助确认因子、预处理规则、周期参数和特征版本
  - 现在还能直接勾选主判断因子和辅助因子，研究评分和解释会真正跟着变化
- 研究到执行一体化工作台现在已完成第三步：
  - 新增策略研究工作台 `/research`
  - 新增策略研究工作台接口 `GET /api/v1/research/workspace`
  - 页面会直接显示研究模板、标签定义、训练窗口、当前模型、实验参数，并在同页提供“研究训练 / 研究推理”入口
  - 现在还能直接修改标签阈值、持有窗口、研究模板和模型，保存后下一轮研究会按新参数运行
- 研究到执行一体化工作台现在已完成第四步：
  - 新增回测工作台 `/backtest`
  - 新增回测工作台接口 `GET /api/v1/backtest/workspace`
  - 页面会直接显示净收益、成本影响、最大回撤、动作段统计、成本模型和候选回测对比
  - 现在还能直接修改手续费和滑点，净收益和回测门会按新口径重算
- 研究到执行一体化工作台现在已完成第五步：
  - 新增评估与实验中心 `/evaluation`
  - 新增评估与实验中心接口 `GET /api/v1/evaluation/workspace`
  - 页面会直接显示实验排行榜、推荐原因、淘汰原因、样本外稳定性和最近实验摘要
  - 现在还能直接修改进入 `dry-run / live` 的门槛，候选排序和自动小额 `live` 放行都会按新门槛执行
- 研究到执行一体化工作台现在已完成第六步：
  - 策略页和任务页已经补上回到研究链的入口
  - 页面会直接给出“去研究 / 去回测 / 去评估”的下一步动作
  - 登录回跳和站内跳转已经补齐新工作台路径，不会再悄悄退回旧页面
- 当前第一批工作台配置能力也已经接通：
  - 数据工作台可以改研究标的、主标的、时间周期、样本长度、滚动窗口 / 固定日期范围
  - 特征工作台可以改主判断因子、辅助因子、缺失值处理策略
  - 研究工作台可以改研究模板、模型、标签阈值、持有窗口、研究权重和置信度
  - 回测工作台可以改手续费和滑点
  - 评估中心可以改 `dry-run / live` 门槛，并直接比较最近两轮实验
  - 自动化页可以改连续失败阈值、同步陈旧阈值、自动暂停和复盘保留数量
- 研究动作反馈这轮也补上了：
  - `运行 Qlib 信号流水线`
  - `研究训练`
  - `研究推理`
  - 现在都会先进入后台运行，再在页面里显示当前阶段、进度条、预计时长和结果去向
- 当前统一工作台配置接口：
  - `GET /api/v1/workbench/config`
  - `POST /api/v1/workbench/config`
  - 用途：统一保存数据范围、因子组合、研究参数、回测参数和 `dry-run / live` 门槛
- 当前工作台配置已经补上最后一层安全收口：
  - 工作台不可用时，页面会直接禁用保存按钮，不再拿 fallback 默认值覆盖真实配置
  - 研究页和评估页会明确提示“当前结果与配置是否对齐”
  - 默认因子和“手动清空因子”已经分开处理，未配置会走协议默认值，明确清空才真的为空
  - 研究模板 `single_asset_timing_strict` 现在会真正影响规则门和评分，而不是只改页面文案
- 研究动作现在已补上后台运行状态：
  - `运行 Qlib 信号流水线`、`研究训练`、`研究推理` 不再只让按钮一直转圈
  - 信号页和研究页会直接显示当前阶段、预计时长和完成后该去哪里看结果
  - 后端会记住最近几次耗时，页面里的预计时长会随真实运行逐步收敛
