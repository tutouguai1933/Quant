...
### 17. 又一根因：logging 锁竞争阻塞事件循环（08-25）
- **py-spy 现行抓获**：主线程卡在 logging acquire——性能监控每 60 秒在事件循环里打全接口统计日志，而 logging 内部锁被其他线程的慢文件 IO（同步 RotatingFileHandler）长期持有 → 主线程等锁阻塞事件循环 → WebSocket 断开+数据加载失败
...
### 19. 波动率突破策略回测（08-25，不采纳）
- 策略：振幅过滤(>1%) + 放量突破追击 + 双向合约 2x 杠杆，止损 1.5%/止盈 4%
- 回测（16 币一年 4h）：TRAIN +62%（388 次）→ TEST -59.2%（111 次，胜率 38%→30.6%）
- 结论：典型过拟合，样本外无正期望 → **不采纳**；做空方向同样亏 -49%
- 教训：EP004 比赛的教训在自己系统上复现——训练集调参的漂亮结果不可信，必须 OOS 验证
- 脚本留存 scripts/run_breakout_backtest.py（参数可调，可做其他变体实验）


### 20. 第二阶段数据源接入（08-25）
- **新增采集**：资金费率历史（币安 fapi，8h 粒度 ~333 天）+ Fear & Greed Index（alternative.me，每日全历史）
- 存储：.runtime/derivatives/*.jsonl 增量追加（与衍生品数据同目录）
- 同步：挂 kline_sync_scheduler 循环（15 分钟/轮），失败单边降级不影响其他
- **目的**：与衍生品数据一起积累 2-4 周 → 全面有效性研究 → 有效信号集成特征 → OOS 考核
- 注意：Fear&Greed 当前 74(Greed) 与模型看跌(0.37) 相反——多维度信息交叉正是模型提升的素材
