...
- 已追加第二轮代码层面审阅：补充共享组件绕过代理、回测曲线真实性、运维动作认证、服务器 VPN/DNS 异常和 WebSocket 推送异常。
- 已按用户要求移除无关视口审阅内容，并追加第三轮全项目代码审阅：重点补充前端 API 规范、后端认证边界、fallback 掩盖真实故障、测试有效性、部署一致性和文件规模问题。
