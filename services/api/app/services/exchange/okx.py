...
class OKXExchange(ExchangeBase):
    ...
    使用 ccxt 统一接口实现 OKX 交易所的所有操作。
    """

    name = "okx"
    display_name = "OKX"
    description = "OKX 交易所适配器，支持现货和合约交易"
    ...
    async def initialize(self) -> bool:
        """初始化 OKX 连接。"""
        if ccxt is None:
            logger.error("ccxt 库未安装，无法初始化 OKX 交易所")
            return False
        try:
        ...
        for pos in positions_data:
            ...
            positions.append(
                Position(
                    ...
                    side="long" if float(pos.get("side", "long") == "long") else "short",
                    size=size,
    ...
    async def cancel_order(self, order_id: str, symbol: str | None = None) -> bool:
        ...
        try:
            ...
            logger.info("OKX 订单撤销成功: %s", order_id)
            return True
        except Exception as e:
            logger.error("OKX 订单撤销失败: %s - %s", order_id, e)
            return False
    ...
    async def get_order(self, order_id: str, symbol: str | None = None) -> Order | None:
        ...
        try:
            ...
            logger.error("OKX 查询订单失败: %s - %s", order_id, e)
            return None
    ...
    async def set_leverage(self, symbol: str, leverage: int) -> bool:
        ...
        try:
            ...
            logger.info("OKX 杠杆设置成功: %s - %dx", symbol, leverage)
            return True
        except Exception as e:
            logger.error("OKX 杠杆设置失败: %s - %s", symbol, e)
            return False
