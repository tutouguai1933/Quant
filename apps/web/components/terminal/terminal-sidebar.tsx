    ...
    <aside className="terminal-sidebar-root hidden lg:flex flex-col h-screen sticky top-0">
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        {NAV_GROUPS.map((group) => {
          const isAdvanced = group.title === "高级模式";
          const navContent = (
            <>
          ...
          return (
            <div key={group.title} className="mb-2">
              ...
              {isAdvanced ? (
                /* 高级模式默认折叠，面向新手隐藏研究员工具 */
                <details className="terminal-nav-details">
                  <summary className="terminal-nav-summary">
                    研究员工具 ({group.items.length})
                  </summary>
                  <div className="pt-1">{navContent}</div>
                </details>
              ) : (
                navContent
              )}
