  ...
  return (
    <div className="terminal-card p-4">
      <div className="mb-4 space-y-3">
        ...
        <h3 className="text-lg font-semibold tracking-tight text-[var(--terminal-text)]">先看研究解释，再决定是否切到策略页</h3>
        <p className="text-sm leading-6 text-[var(--terminal-muted)]">把研究倾向、准入状态和下一步动作收在一张卡里，不再向下堆叠。</p>
      </div>
      <div className="space-y-5">
        <div className="rounded border border-primary/20 bg-primary/10 p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-2">
              <strong className="block text-base text-[var(--terminal-text)]">{formatText(cockpit.overlay_summary, "暂无可用信号")}</strong>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div className="rounded border border-[var(--terminal-border)] bg-[var(--terminal-panel-deep)] p-4">
            <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--terminal-muted)]">研究门控</p>
            <div className="mt-3">
        ...
        <div className="rounded border border-[var(--terminal-border)] bg-[var(--terminal-panel-deep)] p-4">
          <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--terminal-muted)]">下一步动作</p>
          <p className="mt-2 text-sm leading-6 text-[var(--terminal-text)]">{formatText(nextStep, "先继续观察。")}</p>
        </div>
      </div>
    </div>
  );
...
function MetaLine({
  ...
  return (
    <div className="rounded border border-[var(--terminal-border)] bg-[var(--terminal-panel-deep)] p-4">
      <div className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--terminal-muted)]">
        <Icon className="size-3.5" />
        {label}
      </div>
      <p className="mt-3 text-sm font-medium leading-6 text-[var(--terminal-text)]">{value}</p>
    </div>
  );
}
