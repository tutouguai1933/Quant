...
class ResearchWorkspaceTests(unittest.TestCase):
    ...
    def test_research_workspace_page_mentions_key_sections(self) -> None:
        ...
        self.assertIn("模型和标签怎么影响结果", content)
        self.assertIn("当前模型说明", content)
        self.assertIn("标签方式说明", content)
        self.assertIn("触发基础说明", content)
        self.assertIn("持有窗口说明", content)
        self.assertIn("模型目录", content)
        self.assertIn("当前模型", content)
        self.assertIn("当前标签方式", content)
        self.assertIn("当前触发基础", content)
        self.assertIn("当前持有窗口", content)
        self.assertIn("模型说明", content)
        self.assertIn("标签方式说明", content)
        self.assertIn("触发基础说明", content)
        self.assertIn("持有窗口说明", content)
        ...
        self.assertIn("研究预设", content)
        self.assertIn("baseline_balanced", content)
        self.assertIn("trend_following", content)
        self.assertIn("conservative_validation", content)
        self.assertIn("标签预设", content)
        self.assertIn("balanced_window", content)
        self.assertIn("breakout_path", content)
        self.assertIn("closing_confirmation", content)
        self.assertIn("majority_filter", content)
        self.assertIn("标签目标与止损说明", content)
        self.assertIn("未来命中规则", content)
