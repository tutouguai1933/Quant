...
class FeatureWorkspaceTests(unittest.TestCase):
    ...
    def test_feature_workspace_page_mentions_factor_sections(self) -> None:
        ...
        self.assertIn("类别权重配置", content)
        self.assertIn("trend_weight", content)
        self.assertIn("momentum_weight", content)
        self.assertIn("volume_weight", content)
        self.assertIn("oscillator_weight", content)
        self.assertIn("volatility_weight", content)
        self.assertIn("strict_penalty_weight", content)
        self.assertIn("signal_confidence_floor", content)
