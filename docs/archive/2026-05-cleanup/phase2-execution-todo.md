...
1. `Phase2` 已完成，下一步进入 `Phase3` 预备方向
...
- C2 已完成：自动化工作流现在新增 `runtime_window.story` 和 `resume_status`，任务页会直接说明系统在等什么、最早什么时候继续、为什么当前不能恢复；并且已经把“恢复自动化”和“等待调度窗口”拆成两种不同状态，不再混写成“现在可以恢复”
- C3 已完成：自动化状态现在明确拆成“手动模式 / 暂停 / 人工接管”三种；任务页和策略页会按真实状态展示 `确认后恢复自动化 / 只恢复到 dry-run / 切到手动 / 保持手动`，并且手动模式下误点恢复只会得到 warning，不会再假装成功
...
- C1 已完成：自动化服务现在直接返回 `alert_story`，任务页会优先用它说明“最近发生了什么 / 你现在该做什么 / 去哪里处理”，头号告警与告警处理建议不再主要靠 `code / level` 自己猜
- C4 已完成：自动化健康摘要现在明确拆开“当前活跃告警”和“历史告警”；任务页的头号告警、恢复建议和恢复导航只看活跃告警，历史区继续保留给复盘；顶层兼容字段仍保留历史语义，避免外部调用方静默读错
