...
- 只有研究层整体状态为 `ready` 时，这个接口才会返回候选列表
- 如果训练或推理结果缺失，候选列表会直接清空，不再继续透出旧候选
...
当 `source=qlib` 时，当前除了生成研究信号，还会把研究候选里的 `dry-run` 门槛一起带进信号服务。  
这意味着：

- 不通过研究候选门槛的 `Qlib` 信号不会进入可派发队列
- 研究门槛会跟着这条信号一起保存
- 后续研究结果变化时，不会反向改写旧信号的派发资格
