...
- 当前还新增了统一工作台配置入口：
  ...
  - 由 `WorkbenchConfigService` 统一保存数据、因子、研究、回测和门槛配置，再把它们转换成研究层和自动化层真正消费的环境参数
...
- 工作台配置卡当前有统一安全约束：
  - 当页面处在 `fallback / unavailable` 时，配置卡会直接禁用并提示“先恢复研究接口再保存配置”
  - 研究页和评估页会同时显示 `config_alignment`，告诉你当前页面上的配置和最近研究/评估结果是不是同一轮产物
...
下一步主线已经切到“研究到执行一体化工作台”：

- 把数据层显性化成数据工作台
- 把因子层显性化成特征工作台
- 把回测、评估和实验对比真正显性化到前端
- 继续让执行与自动化页承接研究结果，而不是各自为政

当前已先完成第一步：

- 数据工作台后端入口：`services/api/app/routes/data_workspace.py`
...
- 数据工作台现在还会读取统一工作台配置，把研究标的、主标的、周期和样本长度显性化到页面，并把这些值真正传回研究主链
...
- 特征工作台现在还会读取统一工作台配置，把主判断因子和辅助因子的勾选结果显性化到页面，并让研究评分和解释真正按这些因子运行
...
- 策略研究工作台现在会读取统一工作台配置，把研究模板、模型、标签阈值和持有窗口直接回显到页面；保存后下一轮训练和推理会真的按这里的参数运行
...
- 回测工作台现在会读取统一工作台配置里的手续费和滑点，研究层回测和页面显示用的是同一套成本口径
...
- 评估与实验中心现在会读取统一工作台配置里的 `dry-run / live` 门槛，候选排行和自动化 live 放行也共用这套门槛
- 评估与实验中心现在还会显性化：
  - 当前结果和当前配置是否还是同一轮
  - 每个候选卡在哪一层门
  - 最近两轮实验在模型、数据快照、净收益、Sharpe、胜率上的差异
...
- 统一配置中心入口：`services/api/app/routes/workbench_config.py`
- 统一配置中心服务：`services/api/app/services/workbench_config_service.py`
- 前端配置提交入口：`apps/web/app/actions/route.ts`
- 当前统一配置中心负责保存：
  - 数据范围
  - 因子组合
  - 标签方式和模型
  - 回测参数
  - `dry-run / live` 门槛
  - 长期运行参数：连续失败阈值、同步陈旧阈值、自动暂停、复盘保留数量
...
- 数据工作台增加了固定时间窗能力：`window_mode / start_date / end_date`
- 特征工作台增加了缺失值处理策略：`missing_policy`
- 自动化工作台增加了长期运行参数：`operations`
- 所有这些配置都通过同一个统一配置中心下发到研究、回测、自动化主链，而不是各页各自保存一份
