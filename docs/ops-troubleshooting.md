...
### 0.1 ML 模型推理失败（score 恒为 0）

**问题**：自动化周期所有候选 score=0.000，全部被 Score Gate 拦阻

**根因**：两层 Bug：
1. `predictor.py` 用 `str(path)+".txt"` 拼接检查模型文件，但 `model.save()` 用 `path.with_suffix(".txt")` 替换后缀。路径不匹配（`.model.txt` vs `.txt`），模型加载失败，fallback 到启发式评分
2. `qlib_runner.py` 中 `_build_per_symbol_validation` 放在模块级别（0缩进），提前关闭了 `QlibRunner` class，后续方法全部脱离 class

**解决**：
- 统一使用 `with_suffix()` 检查文件存在
- 将模块级函数移到 `class` 定义之后
- 添加 `QUANT_QLIB_RUNTIME_ROOT` 环境变量确保持久化

### 0.2 回测收益恒为负（全部候选被 Backtest Gate 拦阻）

**问题**：15 个候选全部 `non_positive_return`，收益均为 -32%

**根因**：回测对全部测试样本 `future_return_pct` 求和，不区分模型预测买入还是卖出。等于在测"过去市场好不好"，不是"模型选得好不好"

**解决**：推理时用 ML 模型逐行预测，只保留概率≥0.5 的样本做回测

### 0.3 EnhancedStrategy 一直不买入

**问题**：BTC RSI 24.8 超卖、XRP RSI 29.3 超卖，策略不买入

**根因**：入场需要同时满足 4 个条件，不仅是 RSI：
1. RSI < 32 ✅ 但只有瞬时超卖刷新后才满足
2. 价格 > 4H SMA200 ❌ 当前所有币种均低于 4H SMA200
3. 成交量 > 同时段 60% ❌ 凌晨低量，旧方法用 SMA20 对比不准确
4. Freqtrade 处于 PAUSED 状态

**解决**：
- 成交量改为过去 7 天同一小时均量对比
- 阈值从 0.8 降至 0.6
- 重启 Freqtrade 恢复 RUNNING 状态

### 0.4 BNB 仓位 NOTIONAL 卡住

**问题**：0.007 BNB 无法平仓，Binance 报 `NOTIONAL filter failure`

**原因**：BNB 价格约 $640，0.007 BNB ≈ $4.48，低于 Binance $10 最小订单额

**状态**：待处理（不阻止新交易，仅占一个持仓位）

### 0.5 RSI 概览定格在旧数据

**问题**：首页 RSI 定格在 05-06 08:00

**原因**：RSI 缓存一次写入后永不过期

**解决**：`RsiCacheService.get()` 增加 5 分钟 TTL，过期自动重新计算
